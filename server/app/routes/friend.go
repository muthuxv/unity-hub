...
func FriendRoutes(r *gin.Engine) {
	...
	r.GET("/friends/users/:id", controllers.TokenAuthMiddleware("user"), controllers.IsOwner(), services.GetFriendsByUser())
	r.GET("/friends/pending/:id", controllers.TokenAuthMiddleware("user"), controllers.IsOwner(), services.GetPendingFriendsByUser())
